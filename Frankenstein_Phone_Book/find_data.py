# Поиск данных в файлах  Phonebook.csv и Phonebook.txt
# А так же удаление данных
...
def look():  
    ...
    file = 'Phonebook.csv'
    if path.exists(file):
        with open(file, 'r', encoding='utf-8') as text:  # encoding перевод кириллицы в удобоваримый формат
            count = False
            text_csv = text.readlines()
            for i, v in enumerate(text_csv):  # enumirate получение индекса(i) и значения(v)
                if search in v:
